...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .sidebar-header h2 { font-size: 28px; margin-bottom: 10px; }
        .sidebar-header h2 span { color: #ff6b6b; }
        .sidebar-header p { color: #a4b0be; font-size: 14px; }
        ...
        .stat-card h3 { color: #7f8c8d; font-size: 14px; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 10px; }
        .stat-value { font-size: 32px; font-weight: bold; color: #2c3e50; margin-bottom: 5px; }
        .stat-trend { font-size: 13px; display: flex; align-items: center; gap: 5px; }
        ...
        .quick-actions h3 { color: #2c3e50; margin-bottom: 20px; }
        ...
        .view-all { color: #ff6b6b; text-decoration: none; font-size: 14px; }
        table { width: 100%; border-collapse: collapse; }
        th { text-align: left; padding: 12px; color: #7f8c8d; font-weight: 500; font-size: 13px; text-transform: uppercase; border-bottom: 2px solid #e9ecef; }
        ...
        .badge.pending { background: #fff3e0; color: orange; }
        ...
        .status-connected { background: #4ecdc4; box-shadow: 0 0 10px #4ecdc4; }
        .notification {
            position: fixed;
            top: 20px;
            right: 20px;
...
<body>
    <div class="dashboard">
        ...
        <div class="content">
            <div class="header">
                <h1>Dashboard <span><?php echo date('F d, Y'); ?></span></h1>
            ...
            <div class="stats-grid">
                <div class="stat-card">
                    ...
                    <div class="stat-trend <?php echo $revenue_growth >= 0 ? 'trend-up' : 'trend-down'; ?>">
                        <?php echo $revenue_growth >= 0 ? '↑' : '↓'; ?> <?php echo abs($revenue_growth); ?>% vs last month
                    </div>
                </div>
                <div class="stat-card">
                    <div class="stat-icon">📦</div>
                    ...
                    <div class="stat-trend"><?php echo $today_orders; ?> today</div>
                </div>
                <div class="stat-card">
                    <div class="stat-icon">👥</div>
                    <h3>Customers</h3>
            ...
            <div class="quick-actions">
                ...
                <div class="action-buttons">
                    <a href="products.php?action=add" class="action-btn btn-add">➕ Add New Product</a>
                    <a href="categories.php" class="action-btn btn-add">📁 Add Category</a>
                    <a href="orders.php?status=pending" class="action-btn btn-pending">📦 Pending Orders (<?php echo $pending_orders; ?>)</a>
                    <a href="users.php" class="action-btn btn-users">👥 Manage Users</a>
                    <a href="audit.php" class="action-btn btn-logs">📋 Audit Logs</a>
                </div>
            </div>
            ...
            <div class="system-status">
                <div class="status-item"><span class="status-indicator status-connected"></span><strong>WebSocket:</strong> <span id="ws-status-text">Connected</span></div>
                <div class="status-item"><span class="status-indicator status-connected"></span><strong>Database:</strong> Connected</div>
                <div class="status-item"><strong>Server Time:</strong> <?php echo date('Y-m-d H:i:s'); ?></div>
                <div class="status-item"><strong>PHP Version:</strong> <?php echo phpversion(); ?></div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        new Chart(salesCtx, {
            ...
            data: {
                ...
                datasets: [
                    { label: 'Revenue (₱)', data: <?php echo json_encode($revenues); ?>, borderColor: '#ff6b6b', backgroundColor: 'rgba(255,107,107,0.1)', yAxisID: 'y-revenue', tension: 0.4 },
                    { label: 'Orders', data: <?php echo json_encode($order_counts); ?>, borderColor: '#4ecdc4', backgroundColor: 'rgba(78,205,196,0.1)', yAxisID: 'y-orders', tension: 0.4 }
                ]
            },
            options: {
                responsive: true, maintainAspectRatio: false,
                scales: { 'y-revenue': { type: 'linear', position: 'left', title: { display: true, text: 'Revenue (₱)' } }, 'y-orders': { type: 'linear', position: 'right', title: { display: true, text: 'Number of Orders' }, grid: { drawOnChartArea: false } } }
            }
        });
        ...
        new Chart(ordersCtx, {
            type: 'doughnut',
            data: { labels: <?php echo json_encode($statuses); ?>, datasets: [{ data: <?php echo json_encode($status_counts_data); ?>, backgroundColor: ['orange','#2196f3','#4caf50','#f44336'], borderWidth: 0 }] },
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' }, tooltip: { callbacks: { label: function(context) { return `${context.label}: ${context.raw} orders (${<?php echo json_encode($status_percentages); ?>[context.dataIndex]}%)`; } } } } }
        });
        
        const ws = new WebSocket('ws://localhost:8080');
        ws.onopen = function() { document.getElementById('ws-status-text').innerHTML = 'Connected'; };
        ws.onclose = function() { document.getElementById('ws-status-text').innerHTML = 'Disconnected'; };
        ws.onmessage = function(event) {
            const data = JSON.parse(event.data);
            const notification = document.getElementById('notification');
            const div = document.createElement('div');
            div.className = 'notification';
            div.textContent = '🔔 ' + (data.message || 'New update received');
            notification.appendChild(div);
            setTimeout(() => div.remove(), 3000);
            if(data.type === 'new_order') setTimeout(() => location.reload(), 2000);
        };
    </script>
</body>
</html>